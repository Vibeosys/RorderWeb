...
<div class="x_content">
    <?php if (isset($menus)) { ?>
        ...
        <?php if ($menu->fbTypeId and $fbType) {
            $key = $menu->fbTypeId;
            echo $this->Html->image('menu/' . $fbType->$key . '.png', ['class' => "veg-non-veg center-block"]);
        } ?></td>
                    <td>
