...
class RecipeItemMasterTable extends Table {
    ...
    public function getStock($restaurantId) {
        ...
        $allRecipItems = $this->connect()->find('all', array('fields' => $fields, 'conditions' => $conditions))
                ->join($joins);
    ...
    public function getRecipeItems($restaurantId, $type) {
        ...
        if ($allRecipItems->count()) {
            foreach ($allRecipItems as $recipeItems) {
                $response[$counter++] = new UploadDTO\RecipeItemMaterInsertDto(
                        $recipeItems->ItemName, $recipeItems->Category, $recipeItems->UnitId, $recipeItems->SLevel, $recipeItems->RorderLevel, $recipeItems->QtyInHand, $recipeItems->ItemId, $recipeItems->Unit);
            }
        }
        return $response;
    }
        ...
        try {
            ...
            if ($allItems->count()) {
                foreach ($allItems as $item) {
                    $response[$counter++] = new DownloadDTO\RecipeItemMasterShortDto(
